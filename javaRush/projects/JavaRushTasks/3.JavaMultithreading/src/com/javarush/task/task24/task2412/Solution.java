    ...
    public static void sort(List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                ...
                int sortResult = name1.compareTo(name2);

                // if name's the same - sort by date
                if (sortResult == 0) {
                    Date date1 = (Date) stock1.get("date");
                    Date date2 = (Date) stock2.get("date");
                    SimpleDateFormat df = new SimpleDateFormat("yyyyMMdd");

                    sortResult = df.format(date1).compareTo(df.format(date2));

                    // if date's the same - sort by profit
                    if (sortResult == 0) {
                        ...
                        if (profit1 < profit2)      sortResult = 1;
                        else if (profit1 > profit2) sortResult = -1;
                    }
                }
